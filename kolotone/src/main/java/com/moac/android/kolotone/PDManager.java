...
public class PDManager {
    ...
    private static final String TAG = "Kolotone";
    ...
    AudioStatListener receiver = new AudioStatListener();

    public PDManager(final Activity p) {
        this.p = p;
    }
    ...
    public void onResume() {
        ...
        try {
            ...
            toast("Initialising PdAudio");

            PdAudio.initAudio(SAMPLE_RATE, 0, nOut, 1, true);
            PdAudio.startAudio(p.getBaseContext());
            PdBase.setReceiver(receiver);
            PdBase.subscribe("mainlevel");
            PdBase.subscribe("pitch");
        } catch(final IOException e) {
            Log.e(TAG, "Failed to initialise PD " + e.getMessage());
            toast("Failed to Initialise PdAudio");
        }
    }
